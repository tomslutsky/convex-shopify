...
function responseMetadata(response, body) {
...
function graphQLErrors(value) {
    ...
    return value.map((item) => {
        ...
        const path = Array.isArray(error?.path)
            ? error.path.filter((part) => typeof part === 'string' || typeof part === 'number')
            : [];
        return {
...
function componentError(kind, message, status, retryable, metadata) {
...
export const gql = action({
    ...
    handler: async (_ctx, args) => {
        ...
        try {
            ...
            response = await fetch(`https://partners.shopify.com/${encodeURIComponent(organizationId)}/api/${version}/graphql.json`, {
                method: 'POST',
                headers: {
            ...
            throw componentError(controller.signal.aborted ? 'timeout' : 'network', controller.signal.aborted ? 'Shopify Partner API request timed out' : 'Shopify Partner API network request failed', null, true, null);
        }
        finally {
        ...
        if (response.status === 401)
            throw componentError('authentication', 'Shopify rejected the Partner API credential', 401, false, metadata);
        if (response.status === 429)
            throw componentError('throttled', 'Shopify Partner API request was throttled', 429, true, metadata);
        if (!response.ok)
            throw componentError('http', `Shopify Partner API request failed (${response.status})`, response.status, response.status >= 500, metadata);
