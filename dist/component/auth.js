...
const connectionState = v.union(v.object({ status: v.literal('not_installed'), ...stateFields }), v.object({ status: v.literal('ready'), ...stateFields }), v.object({ status: v.literal('missing_scopes'), ...stateFields }), v.object({ status: v.literal('reconnect_required'), ...stateFields }));
const installationSnapshot = v.object({ installed: v.boolean(), ...stateFields });
function classifyState(installed, state, now) {
    if (!installed)
        return { status: 'not_installed', ...state };
    if (state.missingScopes.length > 0)
        return { status: 'missing_scopes', ...state };
    if (state.accessTokenExpiresAt !== null && state.accessTokenExpiresAt <= now && (state.refreshTokenExpiresAt === null || state.refreshTokenExpiresAt <= now))
        return { status: 'reconnect_required', ...state };
...
export const exchangeSessionToken = action({
    ...
    handler: async (ctx, args) => {
        try {
            ...
            let installed = await ctx.runQuery(internal.installations.existsForShop, { shopDomain });
            let stored = await ctx.runQuery(internal.installations.state, { shopDomain });
...
export const state = query({
    ...
    handler: async (ctx, args) => {
        const installed = await ctx.runQuery(internal.installations.existsForShop, { shopDomain: args.shopDomain });
        const stored = await ctx.runQuery(internal.installations.state, { shopDomain: args.shopDomain });
...
export const snapshot = query({
    ...
    handler: async (ctx, args) => await ctx.runQuery(internal.installations.snapshot, { shopDomain: args.shopDomain }),
});
...
export const getState = action({
    ...
    returns: connectionState,
    handler: async (ctx, args) => {
        const stored = await ctx.runQuery(internal.installations.snapshot, { shopDomain: args.shopDomain });
