...
const webhookPool = new Workpool(components
    .webhookWorkpool, {
    maxParallelism: 10,
    retryActionsByDefault: true,
});
const webhookFunctions = internal.webhooks;
...
async function enqueue(ctx, deliveryId) {
    const workId = await webhookPool.enqueueAction(ctx, webhookFunctions.runDelivery, { deliveryId }, {
        onComplete: webhookFunctions.completeDelivery,
        context: { deliveryId },
    });
    await ctx.db.patch('webhookDeliveries', deliveryId, { workId });
}
async function applyShopifyLifecycle(ctx, args) {
...
export const accept = mutation({
    ...
    returns: v.union(v.object({ status: v.literal('accepted'), deliveryId: v.id('webhookDeliveries') }), v.object({ status: v.literal('duplicate'), deliveryId: v.id('webhookDeliveries') }), v.object({ status: v.literal('rejected'), reason: v.literal('invalid_lifecycle_payload') })),
    handler: async (ctx, args) => {
        if (args.deduplicate) {
            const existing = await ctx.db
                ...
                .unique();
            if (existing)
                return { status: 'duplicate', deliveryId: existing._id };
        }
        if (!await applyShopifyLifecycle(ctx, args)) {
            return { status: 'rejected', reason: 'invalid_lifecycle_payload' };
        }
...
export const runDelivery = action({
    ...
    handler: async (ctx, args) => {
        ...
        if (!delivery)
            return null;
        await ctx.runMutation(delivery.handler, {
...
export const getDelivery = query({
    args: { deliveryId: v.id('webhookDeliveries') },
    returns: v.union(v.null(), v.object({ ...deliveryArgs, status: v.string() })),
    handler: async (ctx, args) => {
...
export const replay = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (delivery.status !== 'failed')
            throw new Error('Only failed webhook deliveries can be replayed');
