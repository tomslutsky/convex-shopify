...
const requiredWebhookUris = new Map([
...
for (const path of paths) {
  ...
  for (const subscription of subscriptions) {
    const subscriptionTopics = [...(subscription.topics ?? []), ...(subscription.compliance_topics ?? [])]
    for (const topic of subscriptionTopics) {
      const expectedUri = requiredWebhookUris.get(topic)
      if (expectedUri && subscription.uri !== expectedUri) problems.push(`${path} ${topic} must use ${expectedUri}`)
    }
  }
}
