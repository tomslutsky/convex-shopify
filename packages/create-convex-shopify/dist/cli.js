...
export function parseArgs(argv) {
    const options = { templateRef: 'main', yes: false, install: true, setup: true, dryRun: false, help: false };
    for (let index = 0; index < argv.length; index += 1) {
        ...
        else if (flag === '--template-ref')
            options.templateRef = value();
        else if (flag === '--yes')
            options.yes = true;
        else if (flag === '--no-install')
            options.install = false;
        else if (flag === '--no-setup')
            options.setup = false;
...
export function slugify(value) {
    ...
    if (!slug)
        throw new Error('The app name must contain a letter or number.');
    return slug;
}
export function rewriteTemplatePackage(input, appName, resolvedRef) {
    const packageJson = JSON.parse(input);
    if (!packageJson.dependencies?.['@convex-dev/shopify'])
        throw new Error('Template package is missing @convex-dev/shopify');
...
function run(command, args, cwd, capture = false) {
    ...
    if (result.error)
        throw result.error;
    if (result.status !== 0)
        throw new Error(`${command} ${args.join(' ')} failed${capture && result.stderr ? `: ${result.stderr.trim()}` : ''}`);
    return result.stdout?.trim() ?? '';
}
function resolveRef(templateRef) {
    if (/^[0-9a-f]{40}$/.test(templateRef))
        return templateRef;
...
async function promptForName(defaultValue) {
    const useControllingTerminal = !process.stdin.isTTY && existsSync('/dev/tty');
    if (!process.stdin.isTTY && !useControllingTerminal)
        throw new Error('Interactive input requires a terminal. Pass --name and --yes for automation.');
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (!options.dryRun && !/^[0-9a-f]{40}$/.test(resolvedRef))
        throw new Error('Resolved template ref must be a full commit SHA');
    ...
    try {
        ...
        mkdirSync(extracted);
        if (archiveOverride)
            cpSync(archiveOverride, archive);
        else
        ...
        if (!existsSync(join(template, 'package.json')))
            throw new Error('Downloaded monorepo does not contain template/package.json');
        ...
        if (process.env.CREATE_CONVEX_SHOPIFY_TEST_FAIL_AFTER_SCAFFOLD === '1')
            throw new Error('Injected post-scaffold failure');
        if (options.install)
            run('npm', ['install'], target);
        if (options.setup && options.install)
            run('npm', ['run', 'setup', '--', ...(options.yes ? ['--yes'] : [])], target);
        else if (options.setup)
            process.stdout.write('Skipping setup because dependencies were not installed. Run npm install && npm run setup later.\n');
    }
    catch (error) {
        if (createdTarget && !resumableTarget)
            rmSync(target, { recursive: true, force: true });
        else if (resumableTarget)
