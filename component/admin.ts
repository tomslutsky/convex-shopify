...
function throwComponentError(error: unknown): never {
...
function reconnectAfterRejectedRefresh(metadata: ShopifyResponseMetadata | null): never {
...
export async function withForcedCredentialRefresh<TResult>(
  initialConnection: () => Promise<ShopifyConnection>,
  forcedConnection: () => Promise<ShopifyConnection>,
  request: (connection: ShopifyConnection) => Promise<TResult>,
): Promise<TResult> {
  const connection = await initialConnection()
  try {
    return await request(connection)
  } catch (error) {
    if (!(error instanceof ShopifyTransportError) || error.kind !== 'authentication') throw error
  }
...
export const gql = action({
  ...
  handler: async (ctx, args) => {
    try {
      return await withForcedCredentialRefresh(
        () => ensureFreshConnection(ctx, args.shopDomain),
        () => ensureFreshConnection(ctx, args.shopDomain, { forceRefresh: true }),
        (connection) => graphql(connection, args.query, args.variables),
      )
    } catch (error) {
