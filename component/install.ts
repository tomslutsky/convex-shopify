...
export const reconcileScopes = mutation({
  ...
  handler: async (ctx, args): Promise<{ installed: boolean; changed: boolean; scopes: Array<string> }> =>
    await ctx.runMutation(internal.installations.reconcileScopes, args),
})
...
export const reencrypt = action({
  ...
  handler: async (ctx, args): Promise<{ processed: number; migrated: number; nextCursor: string | null; isDone: boolean }> => {
    const batchSize = args.batchSize ?? DEFAULT_BATCH_SIZE
    if (!Number.isSafeInteger(batchSize) || batchSize < 1 || batchSize > MAX_BATCH_SIZE) throw new Error(`batchSize must be an integer between 1 and ${MAX_BATCH_SIZE}`)
    const currentVersion = credentialKeyring().activeVersion
    const page: {
      page: Array<{ _id: Id<'offlineSessions'>; _creationTime: number; encryptedAccessToken: string; tokenIv: string; tokenKeyVersion: string; encryptedRefreshToken?: string; refreshTokenIv?: string; credentialGeneration: number }>
      isDone: boolean
      continueCursor: string
    } = await ctx.runQuery(internal.installations.reencryptPage, { paginationOpts: { cursor: args.cursor ?? null, numItems: batchSize } })
    let migrated = 0
    for (const row of page.page) {
      if (row.tokenKeyVersion === currentVersion) continue
      ...
      let refresh: { encryptedRefreshToken: string; refreshTokenIv: string } | null = null
      ...
      if (!args.dryRun) {
        const persisted: boolean = await ctx.runMutation(internal.installations.persistReencrypted, { sessionId: row._id, expectedTokenKeyVersion: row.tokenKeyVersion, expectedGeneration: row.credentialGeneration, ...encrypted, ...(refresh ?? {}) })
