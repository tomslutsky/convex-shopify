...
export async function verifyShopifySessionToken(sessionToken: string) {
  ...
  if (typeof dest !== 'string' || typeof sub !== 'string' || !/^\d+$/.test(sub) || typeof iss !== 'string' || typeof exp !== 'number' || typeof nbf !== 'number') throw new Error('Invalid Shopify session-token identity or timing claims')
  let destination: URL
  try {
  ...
  if (destination.protocol !== 'https:' || destination.username || destination.password || destination.port || destination.pathname !== '/' || destination.search || destination.hash || !isShopDomain(destination.hostname)) throw new Error('Invalid Shopify session-token destination')
  const shopDomain = destination.hostname.toLowerCase()
  if (iss !== `https://${shopDomain}/admin`) throw new Error('Invalid Shopify session-token issuer')
...
export class ShopifyTokenRequestError extends Error {
  constructor(
    public readonly kind: TokenFailureKind,
    message: string,
...
function tokenBody(body: unknown): Record<string, unknown> {
  return typeof body === 'object' && body !== null && !Array.isArray(body) ? body as Record<string, unknown> : {}
}

function parseExpiringOfflineToken(body: unknown, status: number): ExpiringOfflineToken {
...
function rejectedTokenError(body: unknown, status: number, refresh: boolean) {
  const value = tokenBody(body)
  ...
  if (status === 429 || status >= 500) return new ShopifyTokenRequestError('transient', `Shopify token endpoint is temporarily unavailable (${status})`, status, true)
  if (refresh && (detail.includes('expired') || detail.includes('expiration'))) return new ShopifyTokenRequestError('expired_refresh_token', 'Shopify refresh token has expired; reconnect the installation', status, false)
  if (refresh && status === 401 && code === 'invalid_request' && description.includes('This request requires an active refresh_token')) return new ShopifyTokenRequestError('invalid_refresh_token', 'Shopify rejected the inactive refresh token; reconnect the installation', status, false)
...
async function requestOfflineToken(shopDomain: string, params: URLSearchParams, refresh: boolean, options: RequestOptions = {}): Promise<ExpiringOfflineToken> {
  ...
  let lastTransient: ShopifyTokenRequestError | null = null
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      if (response.ok) return parseExpiringOfflineToken(body, response.status)
      ...
      if (error instanceof ShopifyTokenRequestError) throw error
      ...
      if (attempt + 1 >= attempts) throw lastTransient
      const exponential = Math.min(250 * 2 ** attempt, 4_000)
...
export async function exchangeOfflineToken(input: { apiKey: string; apiSecret: string; sessionToken: string; shopDomain: string }, options?: RequestOptions): Promise<ExpiringOfflineToken> {
...
export async function refreshOfflineToken(input: { apiKey: string; apiSecret: string; refreshToken: string; shopDomain: string }, options?: RequestOptions): Promise<ExpiringOfflineToken> {
...
export function isShopDomain(value: string | null): value is string {
  return value !== null && /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.myshopify\.com$/i.test(value)
}

export async function validShopifyWebhook(body: ArrayBuffer, provided: string | null) {
