...
export function missingScopes(grantedScopes: string): Array<string> {
...
export const upsert = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) await ctx.db.patch('offlineSessions', existing._id, { ...value, updatedAt: now })
    else await ctx.db.insert('offlineSessions', { shopDomain: args.shopDomain, ...value, installedAt: now, updatedAt: now })
    return null
  },
})
...
export const persistReencrypted = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!session || session.tokenKeyVersion !== args.expectedTokenKeyVersion || (session.credentialGeneration ?? 0) !== args.expectedGeneration) return false
...
export const persistRefreshed = internalMutation({
  ...
  handler: async (ctx, args) => {
    const session = await ctx.db.query('offlineSessions').withIndex('by_shopDomain', (q) => q.eq('shopDomain', args.shopDomain)).unique()
    if (!session || (session.credentialGeneration ?? 0) !== args.expectedGeneration) return false
